...
func (source wakeRepairHandoffSource) validate() error {
	if source.schema != wakeRepairHandoffSchema {
		return fmt.Errorf(
			"wake repair handoff source schema %d unsupported: upgrade amq or restart the newer wake under the matching version",
			source.schema,
		)
	}
...
func sourceFromWire(wire wakeRepairHandoffSourceWire) (wakeRepairHandoffSource, error) {
	...
	if wire.Schema == wakeRepairHandoffSchemaV1 {
		return wakeRepairHandoffSource{}, fmt.Errorf(
			"wake repair handoff source schema %d unsupported: inbox parent directory identity is not bound; restart that wake from its owning terminal so it re-announces at schema 2",
			wire.Schema,
		)
	}
	source := wakeRepairHandoffSource{
