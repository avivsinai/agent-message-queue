...
// re-generated by a successor while it exited is reported as a replacement
// residue (the successor's lock is never touched). Recorded explicitly so the
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantResidue {
				...
				if removed || len(residues) != 1 || residues[0] != wakeLockResidueReplacement {
					t.Fatalf("successor during graceful exit = (removed=%v, err=%v, residues=%v), want a replacement residue and no removal", removed, err, residues)
				}
