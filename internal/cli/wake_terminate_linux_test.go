...
// The refusal stays for the genuinely ambiguous state: the lock is gone after
// our SIGTERM but the process is still alive past the kill-confirm window. No
// SIGKILL is sent to a process whose lock we no longer hold.
func TestTerminateWakePidfdStillRefusesWhenLockGoneAndProcessAlive(t *testing.T) {
	root := secureTempDirForTest(t)
	lockPath := writeWakeLockForTest(t, root, "codex", wakeLock{
		PID:          4242,
		ProcessStart: "start-1",
	...
	attempted, err := terminateWakePidfdWithAuthorization(agentDir, expected, nil, true, 77)
	if err == nil || !errors.Is(err, errWakeTerminationAuthorizationLost) ||
		!strings.Contains(err.Error(), "wake lock is missing before SIGKILL") {
		t.Fatalf("lock gone + process alive error = %v, want authorization-lost refusal", err)
	}
	if !attempted {
		t.Fatal("refusal after a delivered SIGTERM must still report the attempt")
	}
	if len(signals) != 1 || signals[0] != unix.SIGTERM {
		t.Fatalf("signals = %v, want exactly one SIGTERM and no SIGKILL", signals)
	}
	if polls != 2 {
		t.Fatalf("polls = %d, want grace then kill-confirm before refusing", polls)
	}
}

// End to end through the orphan retire path (the coop takeover and
// `amq wake retire` entry): a wake whose lock vanishes during its graceful
// exit is reported removed with no error, and a wake whose lock was
// re-generated by a successor while it exited is reported stopped with a
// replacement residue (the successor's lock is never touched). Recorded explicitly so the
// difference between the two sub-cases is a contract, not an accident.
func TestTerminateOrphanedWakeRetiresAfterGracefulExitPastGrace(t *testing.T) {
	for _, tc := range []struct {
		name        string
		successor   bool
		wantRemoved bool
		...
		{name: "lock removed", wantRemoved: true},
		{name: "lock re-generated by a successor", successor: true, wantResidue: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			const pid = 4242
			root := secureTempDirForTest(t)
			...
			stubLinuxPidfd(
				...
				func(_ int, sig unix.Signal, _ *unix.Siginfo, _ int) error {
					signals = append(signals, sig)
					if sig != unix.SIGTERM {
						t.Fatalf("unexpected signal %v after the lock left our hands", sig)
					}
					if tc.successor {
						writeWakeLockForTest(t, root, "codex", wakeLock{
							PID:          4343,
			...
			removed, err := terminateAndRemoveOrphanedWakeLockInDirWithRawConsent(agentDir, inspection, true, nil)
			if len(signals) != 1 {
				t.Fatalf("signals = %v, want exactly one SIGTERM", signals)
			}
			if polls != 2 {
				t.Fatalf("polls = %d, want grace then graceful-exit confirm", polls)
			}
			if tc.wantRemoved {
				if err != nil || !removed {
					t.Fatalf("graceful exit past grace = (removed=%v, err=%v), want (true, nil)", removed, err)
				}
				if _, statErr := os.Lstat(lockPath); !os.IsNotExist(statErr) {
					t.Fatalf("lock still present after retire: %v", statErr)
				}
			}
			if tc.wantResidue {
				residues := wakeLockRemovalResiduesFromError(err)
				// The old wake stopped (removed=true); the lock now on disk is
				// the successor's, so cleanup reports it as a replacement
				// residue instead of touching it.
				if !removed || len(residues) != 1 || residues[0] != wakeLockResidueReplacement {
					t.Fatalf("successor during graceful exit = (removed=%v, err=%v, residues=%v), want stopped + replacement residue", removed, err, residues)
				}
				successor := inspectWakeLock(root, "codex")
				if !successor.Exists || successor.Lock.Generation != "successor-generation" {
					t.Fatalf("successor lock was disturbed: %+v", successor.Lock)
				}
			}
		})
	}
