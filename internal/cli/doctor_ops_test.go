...
func TestRunOpsChecks_ReportsRemoteCompanion(t *testing.T) {
	...
	// exists here), so the P1-1 projection reports the truth: stale +
	// not-held, never a phantom "active".
...
// keeps the recorded state and reports lock "held".
func TestRunOpsChecks_LocklessCompanionReportsStale(t *testing.T) {
	// The lifetime-lock probe is flock-based; the assertion needs a real
	// flock. Windows has no ProbeLifetimeLock (fail-closed unknown), so the
	// stale/not-held projection this test pins cannot be exercised there.
	switch runtime.GOOS {
	case "windows", "plan9", "js":
		t.Skip("flock lifetime locks are unix-only")
	}
	root := secureTempDirForTest(t)
	if err := fsq.EnsureRootDirs(root); err != nil {
		t.Fatalf("ensure root dirs: %v", err)
