...
func TestRunWakeWithLoopBaselinesBeforeReadiness(t *testing.T) {
	...
	err := runWakeWithLoop([]string{
		...
		if cfg.baseline == nil || cfg.baseline.Path == "" {
			t.Fatal("inject-via baseline was not persisted into the owned wake loop")
		}
		if cfg.baselineRequested {
			t.Fatal("persistent inject-via floor must not be replaced by a post-launch snapshot")
		}
		if _, ok := cfg.baseline.Files["stale.md"]; !ok {
			t.Fatal("stale message missing from persistent startup baseline")
		}
		if _, statErr := os.Stat(readyPath); !os.IsNotExist(statErr) {
			t.Fatalf("ready file published before persistent baseline catch-up: %v", statErr)
		}
		if err := os.WriteFile(filepath.Join(inboxNew, "fresh.md"), []byte("fresh"), 0o600); err != nil {
			t.Fatalf("write fresh message: %v", err)
		}
		if _, ok := cfg.baseline.Files["fresh.md"]; ok {
			t.Fatal("message arriving after baseline capture was incorrectly suppressed")
		}
		if err := cfg.onPrepared(); err != nil {
			t.Fatalf("publish readiness: %v", err)
		}
		if _, statErr := os.Stat(readyPath); statErr != nil {
			t.Fatalf("expected ready file after baseline catch-up: %v", statErr)
		}
...
func TestRunWakeWithLoopPersistsInjectViaWakeOwnerFromEnv(t *testing.T) {
	...
	stubInspectWakeProcess(t, func(pid int) wakeProcessInfo {
		...
		case owner.PID:
			return wakeProcessInfo{
				PID: pid, Running: true, StartToken: owner.ProcessStart, BootID: owner.BootID,
			}
		case self:
			return ownerClaimProcess(pid, "wake-self", owner.BootID)
		default:
