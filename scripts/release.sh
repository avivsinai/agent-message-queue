...
- optionally running fmt-check/vet/test/smoke/build
...
Options:
  ...
  --skip-verify      Skip make fmt-check vet test smoke build
...
Examples:
  ./scripts/release.sh 0.28.6
  ./scripts/release.sh 0.28.6 --no-auto-merge
EOF
}
