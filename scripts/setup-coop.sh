...
echo "   # Mandatory: add notify hook to ~/.codex/config.toml:"
echo "   #   notify = [\"python3\", \"/path/to/repo/scripts/codex-amq-notify.py\"]"
echo "   # When notified, drain: amq drain --include-body"
echo "   # Optional: background terminals for /ps visibility only (does not wake Codex)"
echo "   #   [features]"
echo "   #   unified_exec = true"
echo "   #   while true; do amq monitor --peek --timeout 0 --json; sleep 0.2; done"
echo ""
