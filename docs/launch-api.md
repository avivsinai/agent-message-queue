...
<dir>` overrides discovery: the named directory is the project root, `.amqrc`,
`.amq/launch.json`, and `.gitignore` are written there, and upward base
discovery does not redirect writes into a parent repo's live base root. An
existing `.amq/launch.json` in cwd is authority without the flag, so a nested
