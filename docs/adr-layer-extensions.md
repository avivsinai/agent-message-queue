...
All v1 fields are always present. Optional or unconfigured values render as
empty strings (`""`) for string fields and empty objects (`{}`) for object
fields. `in_session` is always `true` or `false`.
...
- The `--from-session` and `--session` arguments must resolve under the same
  `--root` base root. AMQ does not accept cross-base-root pre-boot sends in
  this contract.
...
- AMQ does not route `--from-session` across distinct base roots. Cross-base-root
  pre-boot sends are future work.
