...
func cleanEnv() []string {
	...
	for _, entry := range os.Environ() {
		...
		case "AM_ROOT", "AM_ROOT_ID", "AM_BASE_ROOT", "AM_BASE_ROOT_ID", "AM_SESSION", "AM_ME",
			"AMQ_ACP_TO", "AMQ_ACP_STATE_DIR", "AMQ_ACP_TURN_TIMEOUT",
...
func TestBinaryDoesNotCopyBuzzSecretIntoMailbox(t *testing.T) {
	...
	session := startACP(t, amqACP,
		...
		"AM_ME="+testMe,
		"AMQ_ACP_TO="+testTo,
		// The turn is live in v2; bound it short because no reply is
		// expected and this test asserts only the delivered body.
		"AMQ_ACP_TURN_TIMEOUT=1s",
