...
// stop on clean exit 0. up registers in the keepalive companion registry
// (adapter "remote") under its flock so a second up on the same root is
// refused (TestStoreRejectsSecondOwnerForSameAdapterTarget).
func up(args []string, stdout, stderr io.Writer) (int, error) {
	...
	// Register in the keepalive companion registry under the remote adapter.
	// A second up on the same root is refused by the registry's flock.
	...
	defer func() {
		// On exit, mark the companion detached so a future up can claim it.
		_, _ = store.Forget(entryID)
	}()
	say(stderr, "amq-remote up: supervising serve for root %s (registry %s)\n", c.root, regPath)

	// Serve args: forward --root, --me, and any remaining flags the user
	// passed (e.g. --fake, --poll, --manifest). Strip --self and --registry.
...
// --me plus any non-up flags the user passed.
